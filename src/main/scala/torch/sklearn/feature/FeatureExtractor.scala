...
package torch.sklearn.feature

import torch.sklearn.feature.transformers.Settings
 ...
 */
class FeatureExtractor[M[_]: CollectionType, T] private[feature](
  ...
  // copy constructor
  private[feature] def this(other: FeatureExtractor[M, T]) =
    this(other.fs, other.input, other.settings)

  @transient private[feature] lazy val as: M[(T, ARRAY)] =
    input.cross(fs).map { case (in, spec) => (in, spec.unsafeGet(in)) }

  @transient private[feature] lazy val aggregate: M[ARRAY] = settings match {
    case Some(x) =>
...
class RecordExtractor[T, F: FeatureBuilder: ClassTag] private[feature](
  fs: FeatureSet[T],
  settings: String
) {
